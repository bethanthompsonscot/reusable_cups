...
# Load your data
treatment_1_df <- read_rds("data/mnl_output/mnl_4_model.rds")
treatment_1_covar_df <- read_csv("data/mnl_output/mnl_4_covar.csv") %>% 
  dplyr::filter(`...1` != "b_REF" & `...1` != "b_DIS_REF") %>%
  dplyr::select(-`...1`) %>% 
  dplyr::select(-contains("REF"))
treatment_2_df <- read_rds("data/mnl_output/mnl_3_model.rds")
treatment_2_covar_df <- read_csv("data/mnl_output/mnl_3_covar.csv") %>% 
  dplyr::select(-`...1`) 
...
coefficients_1 <- treatment_1_df$estimate[c(2, 4:8)]
...
print(results)
results %>% write_csv("data/output/cup_option_3_4_comparison.csv")